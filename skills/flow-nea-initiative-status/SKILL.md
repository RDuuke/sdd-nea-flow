...
2. If `.status.yaml` is missing, infer from artifacts (first match wins):
   ...
   | Condition | Phase |
   |---|---|
   | `impact-map.yaml` AND `specs/` non-empty | SPEC |
...
```text
INIT -> INTAKE -> [human-review gate] -> SPEC -> (DECOMPOSE futuro)
```

Special cases:
- If `awaiting_approval: true`, `next_phase` equals `current_phase` and
  `action_context.blocked: true` with reason `"awaiting_approval"`.
- After SPEC, `next_phase` is `DECOMPOSE` (out of scope; report it but do not
  attempt to run it).
